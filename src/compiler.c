...
{
	/* This check is used in case we define a top-level function.
	 * When the latter calls this function, there is no local variables
	 * to mark initialized - the function is bound to a global variable. */
 ...
 * function object on top of the stack.
 * 
 * The metadata specific for a single function (e.g.)
...
funDeclaration(void)
{
	 ...
	 * This provides support for recursive calls. */
	markInitialized();
